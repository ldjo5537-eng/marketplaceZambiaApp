...
class UI:
    ...
    def thank_you_and_stop(message: str = "Merci pour votre temps !"):
        """Affiche un message de remerciement et stoppe le questionnaire."""
        st.info(message)
        if st.button("↩️ Retour à l'accueil"):
            QuestionnaireBase.reset_state()
            st.rerun()
        st.stop()
...
class QuestionnaireBase:
    ...
    - Gère l'état en session (step, answers, excluded, done)
    - Gère la logique d'exclusion à la Q1
    - Fournit la barre de progression et les helpers de navigation
    """
    ...
    # ---------- Soumission ----------
    @classmethod
    def submit(cls, sheet_name: str):
        row = [datetime.now().strftime("%Y-%m-%d %H:%M:%S")] + list(cls.get("answers").values())
        with st.spinner("Enregistrement en cours…"):
            success = SheetRepository.save(sheet_name, row)
        if success:
            cls.mark_done()
            st.rerun()
    ...
    def render_excluded(cls):
        """Écran affiché si le répondant est exclu (Q1 = Non)."""
    ...
    @classmethod
    def render_back_button(cls, col):
        with col:
...
class ClientQuestionnaire(QuestionnaireBase):
    ...
    # --- ÉTAPE 1 : Vérification (Q1 = question filtre) ---
    @classmethod
    def render_step_1(cls):
        with st.container(border=True):
            ...
            # 🔑 LOGIQUE FILTRE : si Non → on arrête tout de suite
            if q1 == "Non":
                cls.mark_excluded()
                st.rerun()
    ...
    def render_step_2(cls):
        with st.container(border=True):
            ...
            q8 = st.selectbox("8. Comment avez-vous découvert le produit ?",
                              ["Publication sur les réseaux sociaux", "Recommandation d'un proche",
                               "Recherche sur Internet", "Publicité", "Je connaissais déjà le vendeur",
                               "Autre"], key="c_q8")
            q9 = st.selectbox("9. Comment avez-vous payé ?",
                              ["Mobile money", "Carte bancaire", "Virement bancaire",
                               "Paiement à la livraison", "Paiement à la collecte", "Autre"], key="c_q9")
            q10 = st.selectbox("10. Comment avez-vous reçu votre commande ?",
                               ["Livraison à domicile", "Livraison sur mon lieu de travail",
                                ...
                                "Je suis allé(e) chercher le produit chez le vendeur", "Autre"], key="c_q10")
        ...
        cls.render_back_button(col_back)
        with col_next:
            if st.button("Suivant ➡️", type="primary", use_container_width=True):
    ...
    def render_step_4(cls):
        ...
        with st.container(border=True):
            ...
            q18 = st.radio("18. Prêt(e) à payer des frais de livraison ?",
                           ["Oui", "Non", "Cela dépend du montant"], horizontal=True, key="c_q18")
            ...
            q21 = st.text_input("21. Quel produit aimeriez-vous trouver sur une telle application ?", key="c_q21")
            q22 = st.text_area("22. Suggestion pour améliorer les achats en ligne en Zambie ?", key="c_q22")
            q23 = st.selectbox("23. Comment avez-vous reçu ce questionnaire ?",
                               ["WhatsApp", "Facebook", "Instagram", "Groupe d'étudiants",
                                "Groupe professionnel", "Recommandation d'un proche", "Autre"], key="c_q23")
    ...
    def render_step_1(cls):
        with st.container(border=True):
            ...
            vq2 = st.selectbox("2. Dans quelle ville votre activité est-elle située ?",
                               ["Lusaka", "Kitwe", "Ndola", "Livingstone", "Kabwe", "Autre"], key="v_q2")
            vq3 = st.selectbox("3. Depuis combien de temps exercez-vous cette activité ?",
                               ["Moins de 6 mois", "6 mois à 1 an", "1–3 ans", "Plus de 3 ans"], key="v_q3")
        ...
        with st.container(border=True):
            ...
            vq12 = st.selectbox("12. Quel modèle préférez-vous ?",
                                ["Commission uniquement sur les ventes", "Abonnement mensuel",
                                 "Commission + services optionnels", "Je ne sais pas encore"], key="v_q12")
            ...
            vq14 = st.radio("14. Prêt(e) à utiliser un système de paiement sécurisé (paiement après confirmation) ?",
                            ["Oui", "Non", "Cela dépend des conditions"], key="v_q14")
            vq15 = st.radio("15. Accepteriez-vous de faire vérifier votre identité et votre activité ?",
                            ["Oui", "Non", "Cela dépend des informations demandées"], key="v_q15")
            vq16 = st.radio("16. Prêt(e) à utiliser un service de livraison proposé par la plateforme ?",
            ...
            vq17 = st.multiselect("17. Qu'est-ce qui vous empêcherait de vendre sur une nouvelle marketplace ?",
                                  ["Les commissions", "Le manque de clients",
                                   ...
                                   "La peur des arnaques", "La complexité de l'application", "Autre"],
                                  key="v_q17")
